...
export const EventSchedulesPage = () => {
  const navigate = useNavigate();
  const { eventId } = useParams();
  const { data: bells = [] } = useFetch(fetchBells, []);
  const { data: events = [] } = useFetch(fetchBellEvents, []);
  const { data: schedules = [], refetch } = useFetch(() => fetchSchedules(eventId), [eventId]);
  ...
  return (
    <>
      <Topbar title={event ? `${event.name} Timings` : 'Event Timings'} />
      <div style={{ display: 'flex', alignItems: 'center', gap: '.75rem', marginBottom: '1rem' }}>
        ...
        {event && (
          <span style={{ color: '#667085', fontSize: 14 }}>
            {event.active ? 'Active now' : 'Inactive'} · {event.isDefault ? 'Default timing' : 'Custom timing'}
          </span>
        )}
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '1.5rem' }}>
        ...
        <ScheduleForm bells={bells} eventId={eventId} onSubmit={handleCreate} />
      </div>
    </>
  );
};
